...
// A caller that names a DB means THAT DB.
//
...
// The CLI fallback forwards the path (passThroughDbEnv), the HTTP fast path
// cannot. So with MNEME_DB_PATH set and a server listening on the default
// port, the two paths quietly answer from different databases, and which one
// you get depends on whether a process happens to be up.
//
// That is not theoretical: multiple agent runtimes share one machine, each
// with its own DB on its own port. The default is the first one — so a hook
// pinned to agent B's DB would inject agent A's memories.
//
